...
def generate(prompt: str, uncond_prompt: str, input_image=None,
    ...
    while torch.no_grad():
        ...
        if do_cfg:
            ...
            # TODO: Fix this
            cond_tokens = tokenizer.batch_encode_plus([prompt], padding='max_length', max_length=77).input_ids # type: ignore
            # (Batch_size, Seq_len)
            cond_tokens = torch.LongTensor(cond_tokens).to(device)
            # (Batch_size, Seq_len) -> (Batch_size, Seq_len, Dim)
            cond_context = clip(cond_tokens)

            uncond_tokens = tokenizer.batch_encode_plus([uncond_prompt], padding='max_length', max_length=77).input_ids # type: ignore
            ...
            tokens = tokenizer.batch_encode_plus([prompt], padding='max_length', max_length=77).input_ids # type: ignore
        ...
        if input_image:
            ...
            # If we are doing text to image, start with the random noise N(0,I)
